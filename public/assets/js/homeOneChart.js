...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Fungsi untuk memuat data dan memperbarui chart dan indikator
    function loadData(year) {
        fetch(`/dashboard/books-data?year=${year}`)
            .then((response) => response.json())
            .then((data) => {
                // Menampilkan data untuk chart
                const options = {
                    series: [
                        {
                            name: "Jumlah Buku",
                ...
                // Update data untuk indikator perubahan
                const booksCurrentYear = data.booksCurrentYear;
                const percentageChange = data.percentageChange;
                const trend = data.trend;
                const trendColor = data.trendColor;

                // Update jumlah buku tahun ini
                document.querySelector(
                    "#booksCurrentYear"
                ).textContent = `${booksCurrentYear} Buku`;
                document.querySelector(
                    "#percentageChange"
                ).textContent = `${percentageChange}%`;

                // Update warna dan icon indikator perubahan
                const indicator = document.querySelector("#trendIndicator");
                indicator.className = `text-sm fw-semibold rounded ${trendColor} border br-success px-10 py-4 line-height-1 d-inline-flex align-items-center gap-1`;
                ...
                indicator
                    ...
                    .setAttribute(
                        "icon",
                        trend === "up" ? "bxs:up-arrow" : "bxs:down-arrow"
                    );
            });
    }
